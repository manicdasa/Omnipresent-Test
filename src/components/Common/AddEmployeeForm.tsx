...
import { renderTooltipWhenInvalid, validateMaxAllowance, validateMinAllowance } from './_Common';
...
let addEmployeeSchema = yup.object().shape({
    ...
    .max(new Date(Date.now()), 'Date of birth cannot be a future date.'),
...
const AddEmployeeForm = ({ formState, handleClose } : EmployeeProps) =>
{
  ...
  return(
    <Modal backdrop="static" show={formState} onHide={handleClose}>
      ...
      <Formik 
        ...
        {
          console.log(Object.fromEntries(Object.entries(values).filter(([_, value]) => value)))
        }}
        ...
        {({ handleSubmit, setFieldValue, isValid, dirty, values }) => 
          <Form onSubmit={(e : React.FormEvent<HTMLFormElement>) => { e.preventDefault(); handleSubmit(); }}>
           ...
           <div className="row-style row-in-form"> 
            ...
            <AdditionalFields setFieldValue={setFieldValue} countryOfWork={values.countryOfWork}/>

            <div className="form-buttons mt-3">
              <Button variant="secondary" onClick={() => { handleClose() }}>Close</Button>
              {!dirty || !isValid ?
                <OverlayTrigger placement="top" delay={{ show: 0, hide: 400 }} overlay={renderTooltipWhenInvalid}>
                  <Button variant="primary" className="btn btn-secondary disabled-button-submit-form">Save Changes</Button> 
                </OverlayTrigger>
              : <Button variant="primary" className="btn btn-secondary btn-submit-form" type="submit">Save Changes</Button>}
            </div>
          </Form>
        }
      </Formik>
